/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/83216*@* */
...
void inflateBehavior() {
  ...
  inflate = new AttractionBehavior(new Vec3D(), inf1, inf2*1.0f, 0.8f);
...
void initPhysics() {
  ...
  for (WingedEdge e : box.edges.values ()) {
    ...
    physics.addSpring(new VerletSpring(a, b, a.distanceTo(b), 0.012f));
  }
}
void fsr(int ix, float val) {
  inf1 = map(val, 0.0, 1.0, 0.0, 2500.0);
  inf2 = map(val, 0.0, 1.0, -0.2, -1.2);
}
